...
void Display::drawText (uint8_t x, uint8_t y, String &text, uint32_t color, uint8_t size) {
  int textLength = text.length() + 1;
  char charText[textLength];
  text.toCharArray(charText, textLength);
  drawText(x, y, charText, color, size);
}
...
void Display::getColorValues(uint32_t color) {
  red = color >> 16;
